...
export async function getVisitStats() {
  ...
  const visitsToday = await db.get(`SELECT COUNT(*) as count FROM visit_stats WHERE visit_date LIKE ?`, `${today}%`);
...
export async function getChatHistory(sessionId) {
  const db = await openDB();
  return await db.all(`SELECT * FROM chat_history WHERE session_id = ? ORDER BY sent_at`, sessionId);
}
