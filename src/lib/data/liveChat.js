...
class LiveChat {
    ...
    try {
      ...
      this.socket.once('answer', async (data) => {
        ...
        this.peerConnection.onicecandidate = (event) => {
          if (event.candidate) {
            this.socket.emit('iceCandidate', { targetId: this.currentCallData.callerId, candidate: event.candidate });
          }
        };
  ...
  endCall() {
    ...
    if (this.remotestream) {
      const remoteMediaElement = this.callType === 'video' ? document.getElementById('remoteVideo') : document.getElementById('remoteAudio');
  ...
  playRemoteStream(stream) {
    let remoteMediaElement = this.callType === 'video' ? document.getElementById('remoteVideo') : document.getElementById('remoteAudio');
    if (!remoteMediaElement) {
