...
<div class="flex items-center justify-between pt-16 pb-8">
  {#if hasPreviousPage}
    <a href={`${basePath}/1`} data-sveltekit-prefetch class="flex items-center gap-2 font-medium text-zinc-700">
      First
    </a>
    <a href={`${basePath}/${currentPage - 1}`} data-sveltekit-prefetch class="flex items-center gap-2 font-medium text-zinc-700">
  ...
  {#if hasNextPage}
    ...
    <a href={`${basePath}/${pagesAvailable}`} data-sveltekit-prefetch class="flex items-center gap-2 font-medium text-zinc-700">
      Last
    </a>
  {/if}
</div>