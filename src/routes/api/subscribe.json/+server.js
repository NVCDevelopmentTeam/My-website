...
export const POST = async ({ request, url }) => {
  try {
    ...
    const existingSubscriber = await db.get('SELECT * FROM subscribers WHERE email = ?', [data.email]);
    ...
    await db.run('INSERT INTO subscribers (email, subscribed_at, unsubscribe_token) VALUES (?, ?, ?)', [
      data.email,
      subscribedAt,
      unsubscribeToken
    ]);
